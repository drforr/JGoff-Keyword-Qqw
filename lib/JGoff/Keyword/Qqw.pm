...
JGoff::Keyword::Qqw - subroutine definitions with parameter lists
...
=head1 SYNOPSIS
 ...
 qqw search($haystack, $needle = qr/^(?!)/, $offset = 0) {
   ...
 }
...
runtime (e.g. because it's called through a reference).

With L<C<sub>|perlsub>, a prototype comes directly after the function name (if
any). C<JGoff::Keyword::Qqw> reserves this spot for the
L<parameter list|/"Parameter list">. To specify a prototype, put it as the
first attribute (e.g. C<qqw foo :(&$$)>). This is syntactically unambiguous
...
=item 1. Required positional parameters
...
=item 2. Optional positional parameters
...
parameters come first:
  ...
  qqw named_rectangle($name, :$width, :$height) {
    ...
  }
 
  named_rectangle("Avocado", width => 0.5, height => 1.2);

=item 4. Optional named parameters
...
specifying a default argument after an equals sign (C<=>):
  ...
  qqw rectangle(:$width, :$height, :$color = "chartreuse") {
    ...
  }
...
=pod
  ...
  qqw get($url, :$cookie_jar = HTTP::Cookies->new(), :$referrer = $url) {
    ...
  }
...
The keywords provided by C<JGoff::Keyword::Qqw> are customizable. Since
C<JGoff::Keyword::Qqw> is actually a L<pragma|perlpragma>, the provided
keywords have lexical scope. The following import variants can be used:

=over

=item C<use JGoff::Keyword::Qqw ':strict'>

Provides the keyword C<qqw> (described below) and enables argument checks so
that calling a function and omitting a required argument (or passing too many
arguments) will throw an error.

=item C<use JGoff::Keyword::Qqw>

Provides the keywords C<qqw> (described below) and enables "lax" mode:
Omitting a required argument sets it to C<undef> while excess arguments are
silently ignored.

=item C<< use JGoff::Keyword::Qqw { KEYWORD1 => TYPE1, KEYWORD2 => TYPE2, ... } >>

Provides completely custom keywords as described by their types. A "type" is
either a string (one of the predefined types C<function>, C<function_strict>)
...
Valid values: One of the predefined types C<function>, C<function_strict>.
...
useful if you only want to override some properties:
  ...
  use JGoff::Keyword::Qqw { defqqw => { defaults => 'qqw', shift => '$this' } };

This example defines a keyword called C<defqqw> that works like the standard
C<fun> keyword, but the implicit object variable is called C<$this> instead
of C<$self>.
...
compile time:
  ...
  use JGoff::Keyword::Qqw { qqw => { defaults => 'function_strict', runtime => 1 } };
  say defined &foo ? "defined" : "not defined";  # not defined
  qqw foo() {}
  say defined &foo ? "defined" : "not defined";  # defined

C<&foo> is only defined after C<qqw foo() {}> has been reached at runtime.
...
C<function_strict> is like C<function>, but with C<< strict => 1 >.

=back

Plain C<use JGoff::Keyword::Qqw> is equivalent to
C<< use JGoff::Keyword::Qqw { qqw => 'function' } >>.

C<use JGoff::Keyword::Qqw qw(:strict)> is equivalent to
C<< use JGoff:Keyword::Qqw { qqw => 'function_strict' } >>.

=head2 Introspection

You can ask a function at runtime what parameters it has. This functionality is
available through the function C<JGoff::Keyword::Qqw::info> (which is not
...
or a L<JGoff::Keyword::Qqw::Info> object describing the parameter list.

Note: This feature is implemented using L<Moo>, so you'll need to have L<Moo>
installed if you want to call C<JGoff::Keyword::Qqw::info> (alternatively, if
L<Moose> is already loaded by the time C<JGoff::Keyword::Qqw::info> is first
called, it will use that instead).

See L<JGoff::Keyword::Qqw::Info> for examples.

=head2 Wrapping C<JGoff::Keyword::Qqw>

If you want to write a wrapper around C<JGoff::Keyword::Qqw>, you only have to
...
affects the file that is currently being compiled.
 ...
 use JGoff::Keyword::Qqw ();
 sub import {
   JGoff::Keyword::Qqw->import;
   # or JGoff::Keyword::Qqw->import(@custom_import_args);
 }
...
(C<...[...]>). Example:
  ...
  qqw foo(Int $n, ArrayRef[Str | CodeRef] $cb) { ... }
...
you are in "strict" mode, C<JGoff::Keyword::Qqw> generates code to make sure
...
L<JGoff::Keyword::Qqw::Info> object returned by
L<C<JGoff::Keyword::Qqw::info>|/Introspection>.
